...
public class UserDaoImpl implements UserDao{
    ...
    public boolean login(String name, String password) {
        ...
        try{
            ...
            //注意查询语句中的单引号和双引号
            System.out.println("use setString func");
            ResultSet rs = DaBaConnect.selectSqlUser("select * from student where name = ?  and password = ?",name,password);
            while (rs.next()){
    ...
    public boolean delete(String id) {
        ...
        int i = DaBaConnect.AddUpdateDeleteByID(sql,id);
        if (i > 0){
            flag = true;
        }
        DaBaConnect.closeConn();
