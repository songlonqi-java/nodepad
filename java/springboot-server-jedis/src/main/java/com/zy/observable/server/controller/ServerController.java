...
import com.zy.observable.server.util.OurJedisUtil;
...
//    @GetMapping("/gateway")
    ...
    @GetMapping("/redisson")
    @ResponseBody
    public AjaxResult redisson(String key, String value) {
        try {
          ...
          /*  ArrayList<String> arr = new ArrayList<String>();
            ...
            arr.add("key12");
            arr.add("key3");
            redissonClient.getList("list1"+key).addAll(arr);*/

            return AjaxResult.success("set ok");
        } catch (Exception e) {
    ...
    @ResponseBody
    public AjaxResult getredisson(String key) {
        try {
            RBucket<Object> bucket = redissonClient.getBucket(key);
           String v = (String) bucket.get();
